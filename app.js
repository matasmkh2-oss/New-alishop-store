...
document.querySelector("#authForm").addEventListener("submit", async (event) => {
  ...
  try {
    if (authMode === "register") {
      ...
      notify("تم إنشاء الحساب بنجاح. تحقق من بريدك إن كان التحقق مفعلاً.", "success");
      if (authDialog?.close) authDialog.close();
    } else {
      // 1. تصحيح الشروط: فحص الاستجابة بدقة وفصل النجاح عن الفشل
      const { data, error } = await supabase.auth.signInWithPassword({ email, password });

      // حالة الفشل: استدعاء إشعار الخطأ فقط هنا
      if (error) {
        notify(error.message || "تعذر إتمام العملية", "error");
        return;
      }

      // حالة النجاح: التحقق من وجود session أو user
      if (data?.session || data?.user) {
        ...
        // 3. منع الأخطاء المتسلسلة: حماية دوال ما بعد تسجيل الدخول حتى لا تطلق خطأ عاماً
        try {
          currentUser = data.user || data.session?.user || null;
          await loadProfile();
          updateSessionUI();
        } catch (chainErr) {
          console.warn("Post-login background sync error (non-fatal):", chainErr);
        }
      } else {
        notify("تعذر إتمام العملية", "error");
        return;
      }
    }
  } catch (error) {
    // 2. حصر رسالة الخطأ داخل catch لحالات الاستثناء غير المتوقعة فقط
    notify(error?.message || "تعذر إتمام العملية", "error");
  } finally {
...
async function loadProfile() {
  ...
  try {
    ...
    if (walletError) {
      console.warn("loadWallet error:", walletError.message);
    }
    state.wallet.balance = wallet?.balance || 0;
  } catch (err) {
    console.warn("loadWallet exception:", err);
    state.wallet.balance = 0;
  }
}
