...
function renderFloatingContacts(){
  ...
  root.innerHTML=`${w?`<a class="floating-contact whatsapp fab-live" href="https://wa.me/${w}" target="_blank" aria-label="واتساب"><svg viewBox="0 0 24 24" fill="currentColor" width="21" height="21"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.52.149-.174.198-.298.297-.497.1-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg></a>`:""}${t?`<a class="floating-contact telegram" href="https://t.me/${esc(t)}" target="_blank" aria-label="تلغرام"><i data-lucide="send"></i></a>`:""}${S.user?`<button class="floating-contact support fab-live" id="openSupportChat" aria-label="الدعم"><i data-lucide="headphones"></i>${S.supportUnread?`<i class="floating-badge">${S.supportUnread>99?"99+":S.supportUnread}</i>`:""}</button>`:""}`;
...
async function openSupportChat(){
  ...
  let{data:messages}=await supabase.from("support_messages").select("*,sender:profiles(full_name,role)").eq("thread_id",thread.id).order("created_at");
  if(!(messages||[]).length&&!thread.is_user_blocked){
    const welcome={id:"welcome-auto",sender_id:null,body:"أهلًا بك في دعم علي شوب 👋\nأنا المساعد الذكي للمتجر. اكتب استفسارك مباشرة وسيتولى فريق الدعم الرد عليك فورًا.\n\nأسئلة سريعة قد تفيدك:\n• لتتبع طلبك: افتح صفحة طلباتي وستجد الحالة وبيانات التسليم\n• لشحن الرصيد: من المحفظة اختر طريقة الدفع المناسبة\n• للكوبونات: أدخل الرمز داخل صفحة المنتج قبل الشراء",created_at:new Date().toISOString(),is_auto:true};
    messages=[welcome];
  }
...
function playNotificationSound(){try{const A=window.AudioContext||window.webkitAudioContext;if(!A)return;const c=playNotificationSound._ctx||(playNotificationSound._ctx=new A());if(c.state==="suspended")c.resume();const t=c.currentTime;[880,1174.66,1567.98].forEach((f,i)=>{const o=c.createOscillator(),g=c.createGain();o.type="sine";o.frequency.value=f;o.connect(g);g.connect(c.destination);const s=t+i*.09;g.gain.setValueAtTime(.0001,s);g.gain.exponentialRampToValueAtTime(.12,s+.02);g.gain.exponentialRampToValueAtTime(.0001,s+.42);o.start(s);o.stop(s+.44)})}catch{}}
function flashNotificationBell(){const btn=$("#notificationButton"),cnt=$("#notificationCount");if(btn){btn.classList.remove("bell-ring");void btn.offsetWidth;btn.classList.add("bell-ring")}if(cnt){cnt.classList.remove("counter-pop");void cnt.offsetWidth;cnt.classList.add("counter-pop")}}
function toastNotificationMessage2(m){const e=document.createElement("div");e.className="toast note-toast";e.innerHTML=`<i data-lucide="bell-ring"></i><span>${esc(m)}</span>`;$("#toastRoot").append(e);refreshIcons();setTimeout(()=>{e.classList.add("leaving");setTimeout(()=>e.remove(),320)},3600)}
...
async function openResetPasswordDialog(){return new Promise(resolve=>{openModal(`<div class="app-dialog"><div class="app-dialog-icon"><i data-lucide="key-round"></i></div><div class="app-dialog-content"><h2>تعيين كلمة مرور جديدة</h2><p>أدخل كلمة المرور الجديدة للمستخدم بعد فتحه رابط الاستعادة من بريده.</p><input id="resetPassword1" type="password" class="app-dialog-input" placeholder="كلمة المرور الجديدة"><input id="resetPassword2" type="password" class="app-dialog-input" placeholder="تأكيد كلمة المرور الجديدة" style="margin-top:10px"><small id="resetPasswordError" class="app-dialog-error hidden"></small></div><div class="app-dialog-actions"><button id="appDialogCancel" class="btn soft">إلغاء</button><button id="appDialogConfirm" class="btn primary">حفظ كلمة المرور</button></div></div>`);const p1=$("#resetPassword1",modal),p2=$("#resetPassword2",modal),err=$("#resetPasswordError",modal);const finish=v=>{closeModal();resolve(v)};$("#appDialogCancel",modal).onclick=()=>finish(null);$("#appDialogConfirm",modal).onclick=()=>{const a=p1.value,b=p2.value;if(a.length<6){err.textContent="يجب أن تكون كلمة المرور 6 أحرف على الأقل";err.classList.remove("hidden");p1.classList.add("invalid");return}if(a!==b){err.textContent="كلمتا المرور غير متطابقتين";err.classList.remove("hidden");p1.classList.add("invalid");p2.classList.add("invalid");return}finish(a)};[p1,p2].forEach(el=>el.addEventListener("input",()=>{err.classList.add("hidden");p1.classList.remove("invalid");p2.classList.remove("invalid")}));refreshIcons();p1.focus()})}
async function completePasswordRecoveryFlow(){const password=await openResetPasswordDialog();if(!password)return;const{error}=await supabase.auth.updateUser({password});if(error)return toast(error.message,"error");toast("تم تحديث كلمة المرور بنجاح");location.hash="#/account"}
...
async function submitAuth(e){e.preventDefault();try{const email=$("#email").value.trim(),password=$("#password").value;const emailDomain=(email.split("@")[1]||"").toLowerCase();
  const TRUSTED_DOMAINS=["gmail.com","googlemail.com","outlook.com","hotmail.com","live.com","msn.com","icloud.com","me.com","mac.com","yahoo.com","ymail.com","rocketmail.com","proton.me","protonmail.com","aol.com","zoho.com","mail.com","gmx.com","gmx.net"];
  if(S.authMode==="register"&&!TRUSTED_DOMAINS.includes(emailDomain))return toast("التسجيل متاح فقط ببريد من مزوّد معروف (Gmail أو Outlook أو iCloud أو Yahoo وغيرها)","error");
  if(S.authMode==="register"){const fullName=$("#fullName").value.trim(),phoneRaw=$("#phone").value.trim(),phone=phoneRaw.replace(/\D/g,"");if(!fullName)return toast("الاسم الكامل مطلوب","error");if(phone.length<8)return toast("أدخل رقم واتساب صحيحًا مع مفتاح الدولة","error");const{error}=await supabase.auth.signUp({email,password,options:{data:{full_name:fullName,phone}}});if(error)throw error}else{const{error}=await supabase.auth.signInWithPassword({email,password});if(error)throw error}auth.close();toast(S.authMode==="register"?"تم إنشاء حسابك — مرحبًا بك في علي شوب":"تم تسجيل الدخول — أهلًا بعودتك")}catch(e){
  let msg=e.message||"تعذر تسجيل الدخول";
  if(/invalid login credentials/i.test(msg))msg="بيانات الدخول غير صحيحة أو أن الحساب غير مفعّل — إن كنت سجلت للتو فجرّب بعد قليل أو تواصل مع الدعم";
  toast(msg,"error");
}}
...
function showPopupAnnouncement(){
  const popup=(S.announcements||[]).find(a=>a.kind==="popup");
  if(!popup)return;
  const seenKey=`alishop_popup_${popup.id}`;
  if(popup.show_once&&localStorage.getItem(seenKey))return;
  if(popup.show_once)localStorage.setItem(seenKey,"1");
  setTimeout(()=>{
    openModal(`<div class="popup-announce">
      <button class="popup-close" data-close aria-label="إغلاق">×</button>
      ${popup.image_url?`<div class="popup-media"><img src="${esc(popup.image_url)}" alt="${esc(popup.title||"إعلان")}"></div>`:`<div class="popup-media popup-media-icon"><i data-lucide="megaphone"></i></div>`}
      <div class="popup-body">
        ${popup.title?`<h2>${esc(popup.title)}</h2>`:""}
        <p>${linkify(popup.message||"")}</p>
        ${popup.button_text?`<a class="btn primary block popup-cta" href="${esc(popup.button_url||"#/products")}">${esc(popup.button_text)}</a>`:""}
      </div>
    </div>`);
    refreshIcons();
  },900);
...
function bindPullToRefresh(){
  ...
  const indicator=document.createElement("div");indicator.className="pull-indicator";indicator.innerHTML=`<i data-lucide="refresh-cw"></i>`;document.body.append(indicator);refreshIcons();
  let startY=0,armed=false;
  const onHome=()=>{const r=location.hash.replace("#/","").split("?")[0];return r===""||r==="home"};
  window.addEventListener("touchstart",e=>{armed=(window.scrollY<=2&&onHome());startY=e.touches[0].clientY},{passive:true});
  window.addEventListener("touchmove",e=>{if(!armed)return;const d=e.touches[0].clientY-startY;if(d>60&&window.scrollY<=2){indicator.classList.add("visible");indicator.style.transform=`translateY(${Math.min((d-60)/2.2,64)}px)`;indicator.classList.toggle("ready",d>190)}else{indicator.classList.remove("visible","ready");indicator.style.transform=""}},{passive:true});
  window.addEventListener("touchend",async()=>{if(!armed)return;armed=false;const ready=indicator.classList.contains("ready");indicator.style.transform="";indicator.classList.remove("ready");if(ready){indicator.classList.add("spinning");refreshIcons();await Promise.all([catalog(true),loadIdentity(),loadPublic()]);await home();indicator.classList.remove("spinning")}else{indicator.classList.remove("visible")}},{passive:true});
}
async function home(){
  ...
  try{
    ...
    <div class="wallet-card">
      <div class="wallet-card-top"><span class="wallet-icon"><i data-lucide="wallet-cards"></i></span><div><small>رصيدك المتاح</small><strong>${money(S.wallet.balance)}</strong></div></div>${Number(S.profile?.discount_percent)>0?`<div class="wallet-discount-row"><i data-lucide="percent"></i><span>خصمك الخاص <strong>${Number(S.profile.discount_percent)}%</strong> مطبَّق تلقائيًا على كل المتجر</span></div>`:""}
    ...
    ${section("منتجات مميزة","أحدث المنتجات المتاحة",`<a class="btn soft" href="#/products">عرض الكل</a>`)}<div class="grid home-grid-compact" id="homeGrid">${featured.map(pcard).join("")||empty("لا توجد منتجات")}</div>
...
function details(id){
  ...
  const userPct=Number(S.profile?.discount_percent)||0;
  const disc=v=>userPct>0?Math.max(v*(1-userPct/100),0):null;
  ...
  <div class="buy-sheet">
    <div class="buy-hero">
      ...
      <div class="buy-hero-main">
        ...
        <h3>${esc(p.name)}</h3>
        <div class="buy-price-line">${packages.length?`<small>يبدأ من</small>${disc(pkgsMin)!=null?`<del>${money(pkgsMin)}</del><strong>${money(disc(pkgsMin))}</strong>`:`<strong>${money(pkgsMin)}</strong>`}`:`<small>السعر</small>${disc(p.price)!=null?`<del>${money(p.price)}</del><strong>${money(disc(p.price))}</strong>`:`<strong>${money(p.price)}</strong>`}`}</div>
        ${Number(S.profile?.discount_percent)>0?`<div class="buy-discount-note"><i data-lucide="percent"></i> خصمك الخاص <strong>${Number(S.profile.discount_percent)}%</strong> سيُطبَّق تلقائيًا على هذا الطلب</div>`:""}
...
async function deleteUser(id){if(isPrimaryAdmin(id))return toast("لا يمكن حذف المدير الأساسي","error");const ok=typeof appConfirm==="function"?await appConfirm({title:"حذف المستخدم",message:"سيتم حذف الحساب وبياناته المرتبطة نهائيًا إذا لم تكن محمية بقيود النظام. هل تريد المتابعة؟",confirmText:"حذف نهائي",cancelText:"إلغاء",icon:"trash-2",danger:true}):confirm("سيتم حذف الحساب وبياناته المرتبطة نهائيًا. هل تريد المتابعة؟");if(!ok)return;const reason=typeof appPrompt==="function"?await appPrompt({title:"سبب حذف المستخدم",message:"يُحفظ هذا السبب في سجل المدير.",placeholder:"سبب الحذف",confirmText:"متابعة الحذف",icon:"trash-2",danger:true,multiline:false}):(prompt("سبب حذف المستخدم:")||"");if(reason===null)return;const{error}=await supabase.rpc("admin_delete_user",{p_user_id:id,p_reason:reason||null});if(error)return toast(error.message,"error");toast("تم حذف المستخدم");adminUsers()}
...
async function adminFaqs(){
  const{data}=await supabase.from("faqs").select("*").order("sort_order");
  const rows=data||[];
  $("#adminContent").innerHTML=`${section("الأسئلة الشائعة","تظهر في أسفل الصفحة الرئيسية",`<button id="addFaq" class="btn primary"><i data-lucide="plus"></i> إضافة سؤال</button>`)}
  <div class="list">${rows.map(f=>`<div class="card item"><div class="item-main"><h3>${esc(f.question)}</h3><p>${esc(f.answer.slice(0,90))}${f.answer.length>90?"…":""}</p></div><div class="item-actions">${badge(f.is_active?"active":"paused")}${iconButton("pencil","تعديل",`data-faq-edit="${f.id}"`)}${iconButton("trash-2","حذف",`data-faq-del="${f.id}"`)}</div></div>`).join("")||empty("لا توجد أسئلة بعد","أضف أول سؤال شائع ليظهر للعملاء.","circle-help")}</div>`;
  const faqForm=(f=null)=>{
    openModal(`<div class="sheet-head"><h2>${f?"تعديل":"إضافة"} سؤال شائع</h2><button data-close>×</button></div><form id="faqFormEl"><label>السؤال<input id="fq" value="${esc(f?.question||"")}" required maxlength="200"></label><label>الإجابة<textarea id="fa" required maxlength="2000">${esc(f?.answer||"")}</textarea></label><label>الترتيب<input id="fo" type="number" min="0" step="1" value="${f?.sort_order??0}"></label><label class="switch-label"><input id="fact" type="checkbox" ${f?.is_active!==false?"checked":""}> مفعّل (يظهر للعملاء)</label><button class="btn primary block">حفظ</button></form>`);
    $("#faqFormEl").onsubmit=async e=>{e.preventDefault();const payload={question:$("#fq").value.trim(),answer:$("#fa").value.trim(),sort_order:+$("#fo").value||0,is_active:$("#fact").checked,updated_at:new Date().toISOString()};const q=f?supabase.from("faqs").update(payload).eq("id",f.id):supabase.from("faqs").insert(payload);const{error}=await q;if(error)return toast(error.message,"error");toast("تم الحفظ");closeModal();adminFaqs()};
    refreshIcons();
  };
  $("#addFaq").onclick=()=>faqForm();
  $$("[data-faq-edit]").forEach(b=>b.onclick=()=>faqForm(rows.find(x=>x.id===b.dataset.faqEdit)));
...
document.addEventListener("click",async e=>{
  const el=e.target.closest("[data-copy-code]");
  if(el){
    try{await navigator.clipboard.writeText(el.dataset.copyCode);toast("تم نسخ الكود: "+el.dataset.copyCode);}
    catch{toast("تعذر النسخ — حدد الكود يدويًا","error");}
    return;
  }
});
