...
async function openSupportChat(){
  ...
  openModal(`<div class="sheet-head"><div><h2>الدعم الفني</h2><p>محادثة مباشرة داخل التطبيق</p></div><button data-close>×</button></div>
    <div id="supportMessages" class="support-messages">${(messages||[]).map(m=>`<div class="support-message ${m.sender_id===S.user.id?"mine":"theirs"}"><div>${esc(m.body)}</div><small>${dt(m.created_at)}</small></div>`).join("")||`<div class="empty-chat">ابدأ المحادثة مع الدعم</div>`}</div>
    <form id="supportSendForm" class="support-send"><input id="supportBody" class="input" placeholder="اكتب رسالتك..." required><button class="icon-action primary"><i data-lucide="send"></i></button></form>`);
  $("#supportSendForm").onsubmit=async e=>{e.preventDefault();const body=$("#supportBody").value.trim();if(!body)return;const{error}=await supabase.from("support_messages").insert({thread_id:thread.id,sender_id:S.user.id,body});if(error)return toast(error.message,"error");$("#supportBody").value="";closeModal();openSupportChat()};
  refreshIcons();
}
...
async function adminSupport(){
  ...
  $("#adminContent").innerHTML=`${section("محادثات الدعم","كل محادثات العملاء داخل التطبيق")}<div class="list">${(data||[]).map(t=>`<div class="card item"><div class="item-main"><h3>${esc(t.profile?.full_name||"-")}</h3><p>${esc(t.subject||"محادثة دعم")} • ${dt(t.updated_at)}</p></div><div class="item-actions">${badge(t.status)}${t.profile?.phone?`<a class="icon-action whatsapp-btn" href="https://wa.me/${String(t.profile.phone).replace(/\D/g,"")}" target="_blank"><i data-lucide="message-circle"></i></a>`:""}${iconButton("messages-square","فتح المحادثة",`data-support-thread="${t.id}"`)}</div></div>`).join("")||empty("لا توجد محادثات")}</div>`;
...
async function openAdminSupportThread(thread){
  const{data:messages}=await supabase.from("support_messages").select("*,sender:profiles(full_name,role)").eq("thread_id",thread.id).order("created_at");
  openModal(`<div class="sheet-head"><div><h2>${esc(thread.profile?.full_name||"عميل")}</h2><p>محادثة الدعم</p></div><button data-close>×</button></div><div class="support-messages">${(messages||[]).map(m=>`<div class="support-message ${m.sender_id===S.user.id?"mine":"theirs"}"><div>${esc(m.body)}</div><small>${dt(m.created_at)}</small></div>`).join("")}</div><form id="adminSupportForm" class="support-send"><input id="adminSupportBody" class="input" placeholder="اكتب الرد..." required><button class="icon-action primary"><i data-lucide="send"></i></button></form>`);
  $("#adminSupportForm").onsubmit=async e=>{e.preventDefault();const body=$("#adminSupportBody").value.trim();if(!body)return;const{error}=await supabase.from("support_messages").insert({thread_id:thread.id,sender_id:S.user.id,body});if(error)return toast(error.message,"error");await supabase.from("support_threads").update({status:"open",updated_at:new Date().toISOString()}).eq("id",thread.id);closeModal();openAdminSupportThread(thread)};refreshIcons()
}
