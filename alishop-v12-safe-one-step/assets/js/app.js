...
async function adminUsers(){let q=supabase.from("profiles").select("id,full_name,phone,role,status,created_at,wallets(balance)",{count:"exact"}).order("created_at",{ascending:false});if(S.query)q=q.ilike("full_name",`%${S.query}%`);if(S.filter)q=q.eq("status",S.filter);const from=(S.page-1)*CONFIG.PAGE_SIZE,{data,count}=await q.range(from,from+CONFIG.PAGE_SIZE-1),r=data||[];$("#adminContent").innerHTML=`${adminHeader("المستخدمون","الرصيد والحظر والدور",`<button id="exportUsers" class="btn soft">تصدير CSV</button>`)}<div class="list">${r.map(u=>{const protectedAdmin=isPrimaryAdmin(u.id);return`<div class="card item"><div class="item-main"><h3>${esc(u.full_name||"-")}${protectedAdmin?` <span class="badge success">المدير الأساسي</span>`:""}</h3><p>${money((u.wallets?.balance??u.wallets?.[0]?.balance??0))} • ${u.role} • ${esc(u.phone||"-")}</p></div><div class="item-actions">${badge(u.status)}${u.phone?`<a class="whatsapp-btn" href="https://wa.me/${String(u.phone).replace(/\D/g,"")}" target="_blank">◉ واتساب</a>`:""}<button class="small" data-user-wallet="${u.id}">الرصيد</button>${protectedAdmin?"":`<button class="small" data-user-role="${u.id}">الدور</button><button class="${u.status==="blocked"?"success":"danger"}" data-user-status="${u.id}" data-current="${u.status}">${u.status==="blocked"?"فك الحظر":"حظر"}</button>`}</div></div>`}).join("")||empty("لا يوجد مستخدمون")}</div>${pager(S.page,count||0,CONFIG.PAGE_SIZE)}`;bindAdminSearch(adminUsers,[["active","نشط"],["blocked","محظور"]]);bindPager(adminUsers);$("#exportUsers").onclick=()=>exportCsv("profiles","users.csv");$$("[data-user-wallet]").forEach(b=>b.onclick=()=>adjustWallet(b.dataset.userWallet));$$("[data-user-role]").forEach(b=>b.onclick=()=>changeRole(b.dataset.userRole));$$("[data-user-status]").forEach(b=>b.onclick=()=>changeStatus(b.dataset.userStatus,b.dataset.current))}
